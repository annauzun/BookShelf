...
function renderBooks() {
    ...
    books.forEach((book) => {
        document
            .getElementById(`updateBook-${book.id}`)
    ...
    books.forEach((book) => {
        document
            .getElementById(`deleteBook-${book.id}`)
            .addEventListener("click", () => {
                deleteBook(book.id)
            })
    })
...
function addBook() {
    //получить значения из полей ввода данных модального окна
